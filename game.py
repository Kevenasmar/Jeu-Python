...
class Game:
    ...
    def draw_highlighted_cells(self, valid_cells):
        ...
        # Create a transparent surface for valid cells
        valid_cell_surface = pygame.Surface((GC.CELL_SIZE, GC.CELL_SIZE), pygame.SRCALPHA)
        valid_cell_surface.fill((255, 255, 0, 180))  # Yellow with 50% transparency
        ...
        if (hover_x, hover_y) in valid_cells:
            ...
            hover_surface.fill((255, 0, 0, 180))  # Yellow with 70% transparency for hover
        ...
        # Update display partially for efficiency
...
def main():
    ...
    WEIGHTS = [80, 40, 40, 10, 5, 25]
    random_seed = random.randint(0, 1000)

    world = World(GC.WORLD_X, GC.WORLD_Y, random_seed)
    tile_map = world.get_tiled_map(WEIGHTS)

    screen = pygame.display.set_mode((GC.WIDTH, GC.HEIGHT + 40))  # Added space for the game log
